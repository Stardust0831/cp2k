...
readonly pr_source_sha="$(git rev-parse "${GITHUB_SHA}^")"
...
git diff --binary "${pr_source_sha}" -- make_cp2k.sh \
  > artifacts/ci-only-source.patch
...
ssh "${ssh_options[@]}" -tt "${GPU_SSH_HOST}" \
  "srun --partition=16V100 --qos=flood-1o2gpu --nodes=1 --ntasks=1 \
   --gres=gpu:V100-SXM2:2 --time=02:20:00 --unbuffered \
   env WORK_ROOT=\"\$HOME/${remote_root}\" SOURCE_SHA=\"${GITHUB_SHA}\" \
   PR_SOURCE_SHA=\"${pr_source_sha}\" \
